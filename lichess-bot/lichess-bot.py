...
def start(li, user_profile, engine_factory, config):
    ...
    control_stream = threading.Thread(target=watch_control_stream, args=(control_queue, li))
    ...
    with logging_pool.LoggingPool(max_games+1) as pool:
        while True:
            ...
            if event["type"] == "exit":
                ...
                gt = threading.Thread(target = play_game, args = (li, game_id, control_queue, engine_factory, user_profile, config))
                gt.daemon = True
                gt.start()
                busy_processes += 1
